  ...
  \begin{itemize}
    \item 由\alert{编码器}和\alert{解码器}两部分组成:
      \begin{itemize}
        ...
        \item 解码器: 尝试从编码 $\B{h}$ 重建原输入信号，$\B{r} = g(\B{h})$.
      \end{itemize}
    \item 对编码 $\B{h}$ 施加约束，训练使得重建信号 $\B{r}$ 的损失
      $L(\B{r}, \B{x})$ 最小.
  ...
  \begin{itemize}
    \item 基于\alert{去噪准则}的全新训练策略 \cite{vincent2008,vincent2010}:
      \begin{enumerate}
        ...
        \item 使重建信号 $\B{r}$ 尽可能地恢复原始输入信号 $\B{x}$.
      \end{enumerate}
...
\begin{frame}{EoR 信号分离效果}
  ...
  \myfigure{%
    height=0.9\textheight,
  }{cdae-eor-pix}{CDAE 重建的 EoR 信号的示例 ($\xi = 0.931$)}
\end{frame}

\begin{frame}
  \vspace{1ex}
  \begin{itemize}
    \item 重建的 EoR 图像的空间结构与强度分布与输入几乎完全相同；
