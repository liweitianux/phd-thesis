...
\begin{itemize}
  ...
  \item \ac{turbulence}越激烈，\ac{turbulence}的速度弥散 \ac{v-turb} 越大，
    对应的 Mach 数 ($\mathcal{M}_t$) 也越大，于是 \ac{coef-diffusion} 也就越大。
\end{itemize}
...
\begin{figure}[htp]
  ...
  \bicaption[逆 Compton 散射、同步辐射和 Coulomb 碰撞的能量损失时标对比]{%
...
另一个对比则利用全天积分的 \SI{1.4}{\GHz} \ac{f-flux}。
...
\begin{table}[htp]
  ...
  \bicaption[前景成分和 EoR 信号在 124、158 和 196 MHz 的亮温度\acs*{rms}值]{%
    射电晕、银河系同步辐射、银河自由—自由辐射、河外点源以及 EoR 信号
...
\begin{figure}[htp]
  ...
  \bicaption[EoR 信号在 \SIrange{120}{200}{\MHz} 的亮温度\acs*{rms}值]{%
