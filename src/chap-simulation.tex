...
目前只有少量几个工作在研究 EoR 前景时包含了\ac{gc}\ac{rh}
\cite{diMatteo2004,gleser2008,jelic2008}，
同时，这些工作对\ac{rh}的建模过于简化，比如
...
银河系\ac{rad-syn}在低频波段的\ac{skymap}通过以
...
我们采用了 Briggs 权重\cite{briggs1995}
...
我们对星系团射电晕构建了一个更物理、更完善的模型，
并据此模拟了射电晕的低频射电\ac{skymap}。
另外，我们模拟了银河系\ac{rad-syn}和\ac{rad-ff}、河外点源、
以及 EoR 信号的\ac{skymap}。
最后，我们采用了 SKA1-Low 的阵列布局，对上述各成分的\ac{skymap}开展模拟观测，
得到包含实际仪器效应的射电图像。
这些模拟的\enquote{观测}图像将在\autoref{chap:halo}用于评估射电晕对 EoR 信号探测的影响，
