...
因此这个态的\ac{dod}为 3，称为\ac{triplet}；
对于 $F = 0$ 态（即下能级），磁量子数 $m$ 只能取 0，因此这个态是\ac{singlet}.

中性氢的 \ac{21cmline}%
首次由 Hendrik Christoffel van de Hulst 在 1945 年提出 \cite{vanDeHulst1945}，
并由 Harold Ewen 和 Ed Purcell 在 1951 年观测到 \cite{ewen1951}.
该谱线的频率是测量最精确的几个物理量之一 \cite{hellwig1970,essen1971}：
\begin{equation}
  \label{eq:hi-line-frequency}
  \nu = \SI{1420405751.7667 +- 0.0009}{\Hz} ,
\end{equation}
对应真空中的波长为：
\begin{equation}
  \label{eq:hi-line-wavelength}
  \lambda = \SI{21.1061140542}{\cm} .
\end{equation}
