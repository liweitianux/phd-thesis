...
\begin{figure}[htp]
  ...
  \bicaption[氢原子的自旋翻转跃迁]{%
    ...
    A hydrogen atom makes a spin-flip transition between the two
...
由于质子和电子的自旋发生相互作用，
氢原子的\ac{ground-state}发生\ac{hyperfine-splitting}而变成两个态：
\begin{itemize}
...
\begin{equation}
  ...
  \acs{freq21cm} =
    \SI[separate-uncertainty=false]{1420405751.7667(9)}{\Hz} ,
\end{equation}
对应真空中的波长为：
\begin{equation}
...
中性氢 \ac{21cmline}的跃迁概率非常低，其\acl{coef-A21}为：
\begin{equation}
  \ac{coef-A21} =
    \SI[separate-uncertainty=false]{2.86888(7)e-15}{\per\second} .
\end{equation}
对应的\ac{em-spontaneous}半衰期为：
\begin{equation}
...
\end{equation}
如果需要考虑中性氢云的\ac{v-peculiar}，则上式应修正为
...
\begin{itemize}
  \item CMB 光子被中性氢吸收或者使中性氢产生\ac{em-stimulated}，
    该过程使 \acs{T-spin} 与 $T_{\R{cmb}}(z)$ 相互耦合；
...
由于缺乏足够的观测证据的约束，上述各阶段的划分仍有很大的不确定性，
相邻阶段之间可能有显著交叠，甚至 $z_{\alpha}$ 和 $z_h$ 的次序可能需要修正
\cite{nusser2005,pritchard2012}。
基于目前有限的观测证据，理论模型显示宇宙的再电离过程从红移 $z \sim 15$ 开始，
...
上述三维 Fourier 变换采用了三维直角坐标系，即要求输入图像 $I(\B{\theta})$
为二维平面（或者能够近似为二维平面）。
大视场是低频干涉阵列的重要优势之一，
...
\begin{figure}[htp]
  ...
  \bicaption[EoR 信号和前景辐射的三维功率谱示意图]{%
...
为球壳的半径。
通过这种方式，EoR 信号的信息被压缩到少量 Fourier \ac{mode}里，
有效地提高了每个\ac{mode}里 EoR 信号的\ac{snr}，
...
因此在三维功率谱中，前景辐射的功率虽然将被较好地约束在频率维度
($\eta$ 或 \ac{k-los})，
但却弥散在整个空间维度 ($\B{u}$ 或 $\B{\ac{k-perp}}$)，
如\autoref{fig:eor-fg-fourier} 右栏所示。
如果简单地对三维功率谱按球壳平均为一维\ac{ps} $\ac{psD}(\ac{k})$，
...
EoR 信号的\ac{ps}可以直接从干涉阵列测量的\ac{vis}数据计算得到，并不需要经过成像。
...
\end{itemize}
关于这两类功率谱分析方法的详细讨论可参见 \citeay{morales2019}。
...
但是前景辐射的涨落幅度仍达数 \si{\kelvin} 至数十 \si{\kelvin}，
远远压制了待测 EoR 信号 \cite{zaroubi2013}。
\autoref{fig:eor-foregrounds} 显示了主要的前景干扰成分。
...
\ac{rfi} 的强度通常会比天空信号高出若干个数量级，并且会实时发生变化 \cite{bentum2011}。
...
对于 \ac{lofar}、\ac{ska} 等采用数字\ac{beam-forming}技术的\ac{phased-array}而言，
...
如何有效地校准仪器，发挥出仪器的设计性能，是目前最迫切的任务之一
...
还需要设计新算法，开发新软件，优化数据处理流程，充分利用大规模并行计算资源
...
基于大规模巡天的研究更加匮乏。
考虑到磁场强度的涨落、高能带电粒子的密度涨落、\ac{ism}的湍流等因素的影响
...
此外，河外点源的\ac{clustering-effect}会改变其功率谱，
...
\begin{figure}[htp]
  ...
  \bicaption[干涉阵列基线的\acs*{chromatic-effect}示意图]{%
    ...
    The background fringes indicates the real part of the visibilities
    of a flat-spectrum point source.
  }
