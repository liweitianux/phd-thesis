...
得到\emph{一维\ac{ps}} $\ac{psD}(\ac{k})$ \cite{morales2004,datta2010}，
...
\begin{figure}[htp]
  ...
  \bicaption[计算二维功率谱的示意图]{%
    计算二维功率谱 $\ac{psD}(\ac{k-perp}, \ac{k-los})$ 的示意图：
...
得到\emph{二维功率谱} $\ac{psD}(\ac{k-perp}, \ac{k-los})$，
...
这些人工源主要有：\ac{am}和\ac{fm}广播、卫星通信、\ac{gps} 信号、对讲机、雷达、等等.
...
\item
\emph{仪器效应:}
大型干涉阵列通常由成千上万根天线组成. 由于生产和安装过程的差异以及随环境和时间的变化，
...
\begin{itemize}
  \item \emph{射电星系 (radio galaxy)}:
    宿主通常为巨型椭圆星系 (giant elliptical galaxy)，
...
\begin{figure}[htp]
  ...
  \bicaption[EoR 窗口和前景楔形的示意图]{%
    二维功率谱 $\ac{psD}(\ac{k-perp}, \ac{k-los})$ 上的 EoR 窗口（左上方绿色区域）
    和前景楔形（右下方灰色区域）的示意图.
    望远镜所能测量的二维功率谱的边界为：
...
即同一条\ac{baseline}的空间分辨率随着频率的增大而提高 \cite{liu2014}.
...
然而，EoR 信号被淹没在强烈的前景干扰之中.
于是，我们介绍了银河系的\ac{rad-syn}和\ac{rad-ff}、河外点源以及河外展源
这四种主要的前景成分，
然后概述了目前的前景处理方法，包括\ac{fg-rm}和\ac{fg-avd}两大类.
\ac{fg-avd}方法依赖于二维\ac{ps}上的 EoR 窗口，
为此，我们基于干涉阵列的\ac{chromatic-effect}%
简要解释了\ac{fg-wedge}和 EoR 窗口的形成原因.
本章内容是本文后续研究的基础.
