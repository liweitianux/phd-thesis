...
\begin{figure}[htp]
  ...
  \bicaption[CDAE 对输入数据的敏感度分布]{%
    采用\acs*{occlusion}方法得到的 CDAE 对输入数据的敏感度分布 $\B{s}$ （蓝线）。
...
$\bar{\rho}_{\R{noft}} = \num{0.628 +- 0.167}$，
显著差于使用了 Fourier 变换预处理数据所得的结果 (\autoref{sec:cdae-results})。
