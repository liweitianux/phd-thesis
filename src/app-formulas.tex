...
两个相邻物体之间的\emph{\acl{distance-comoving} (comoving distance)}
...
\begin{figure}[tbp]
  ...
  \bicaption[三种宇宙距离测量随红移的变化]{%
    \acl*{distance-comoving} $\acs*{distance-comoving}(z)$、
    \acl*{distance-angular} $\acs*{distance-angular}(z)$
    和\acl*{distance-luminosity} $\acs*{distance-luminosity}(z)$
    三种宇宙距离测量从红移 $z = \num{e-3}$ 至 $z = \num{e3}$ 的变化情况.
  }{%
    A comparison among the comoving distance $\acs*{distance-comoving}(z)$,
    angular diameter distance $\acs*{distance-angular}(z)$, and
    luminosity distance $\acs*{distance-luminosity}(z)$,
    from redshift $z = \num{e-3}$ to $z = \num{e3}$.
  }
