...
仅作为天线测量值的一个方便表示。
这个概念被广泛使用的原因主要有 \cite{condon2016}：
\begin{itemize}
...
该项反映了干涉阵列天线的三维分布情况以及成像天区偏离平面近似的程度，
所以这\emph{不是}二维 Fourier 变换。
但是在下述两种常见的特殊情况下，上式可变成二维 Fourier 变换，
从而可以通过逆 Fourier 变换从\ac{vis} \acs{Vis} 获得目标的亮度分布 \ac{I-nu}。
\begin{itemize}
\item
...
\begin{align}
  \acs{Vis}(u,v)
    & = \acs{Vis}(u,v,w) \exp (2\Cpi\Ci\,w)  \\
    & \approx \iint \frac{\ac{I-nu}(l,m)}{\sqrt{1-l^2-m^2}}
...
\hspace{2\ccwd}%
\autoref{eq:vis-approx} 中的近似所引入的相位误差为：
\begin{equation}
  \sigma_p = \Cpi w (l^2 + m^2) .
\end{equation}
为估算 $w$ 的最大值，可设干涉阵列观测一个接近地平线的源，则有：
\begin{equation}
  w_{\R{max}}
    \sim b_{\R{max}} / \lambda
    \approx \acs{bw-synthesized}^{-1} ,
\end{equation}
其中 $b_{\R{max}}$ 为基线的最大长度，
\acs{bw-synthesized} 为\acl{bw-synthesized} [\autoref{eq:bw-synthesized}]。
于是最大相位误差约为：
\begin{equation}
  (\sigma_p)_{\R{max}} \sim
    \Cpi \acs{bw-synthesized}^{-1}
    \left( \frac{\theta_f}{2} \right)^2 ,
\end{equation}
其中 $\theta_f$ 为成像视场大小。
如果要求相位误差 $\sigma_p$ 不超过某个阈值，比如 \SI{0.1}{\radian}，则要求：
\begin{equation}
  \theta_f < \frac{1}{3} \sqrt{\acs{bw-synthesized}} ,
\end{equation}
其中 $\theta_f$ 和 \acs{bw-synthesized} 均以 \si{\radian} 为单位。
参见 \citeay{thompson2017} 的第 3.1.1 节。
...
和 Cotton--Schwab 算法 \cite{schwab1984}，显著地提高了 CLEAN 效率。
