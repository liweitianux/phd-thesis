...
\begin{equation}
  ...
  \right.
\end{equation}
其中 $T_b$ 为系统环境辐射的\ac{T-brightness}.
...
\begin{equation}
  ...
  C_{12} = \frac{g_2}{g_1} C_{21}
    \exp\left( - \frac{h \nu_0}{\acs{kb} T_k} \right) ,
\end{equation}
其中 $T_k$ 为系统（通常为气体）的运动温度 (kinetic temperature).
将\autoref{eq:tex-coefs-b}和\autoref{eq:tex-coefs-c}
代入\autoref{eq:detailed-balance-collison1}，整理可得：
\begin{equation}
  \frac{N_2\,g_1}{N_1\,g_2} =
    \frac{A_{21} + C_{21} \exp\left( -\frac{h \nu_0}{\acs{kb} T_k} \right)
      \left[ \exp\left( \frac{h \nu_0}{\acs{kb} T_b} \right) - 1 \right]
    }{A_{21} \exp\left( \frac{h \nu_0}{\acs{kb} T_b} \right) + C_{21}
      \left[ \exp\left( \frac{h \nu_0}{\acs{kb} T_b} \right) - 1 \right]} .
\end{equation}
...
\end{equation}
从上式可以看出，
如果 $A_{21} \gg C_{21}$，则 $\acs{T-excitation} \to T_b$；
反之，如果 $A_{21} \ll C_{21}$，则 $\acs{T-excitation} \to T_k$.
