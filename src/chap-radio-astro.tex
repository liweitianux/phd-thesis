...
\begin{figure}[htp]
  \centering
  \includegraphics[width=0.7\textwidth]{flux-density}
...
\begin{figure}[htp]
  ...
  \bicaption[天线的主瓣及其宽度的示意图]{%
    天线的主瓣及其\acl*{hpbw} (HPBW) 的示意图.
  }{%
...
\begin{figure}[htp]
  ...
  \bicaption[计算天线的平均接收面积的思想实验]{%
    计算天线的平均接收面积 $\langle A_e \rangle$ 的思想实验.
  }{%
    A thought experiment to calculate the average collection area
    $\langle A_e \rangle$ of an antenna.
    \\来源/Credit:
...
\end{equation}
于是，天线的平均接收面积 $\langle A_e \rangle$ 为：
\begin{equation}
  \label{eq:area-avg}
...
\end{equation}
设天线的有效接收面积为 $A_e$ [\autoref{eq:area-eff}]，
...
\begin{figure}[htp]
  ...
  \bicaption[$(u,v,w)$ 坐标系]{%
    ...
    目标的亮度分布则用\acs*{dc}描述，即 $\ac{I-nu}(\hat{\B{s}}) = \ac{I-nu}(l,m)$，
    其中 $l, m$ 分别为方向矢量 $\hat{\B{s}}$ 对 $u, v$ 轴的投影长度.
  }{%
...
其中 $w$ 轴指向参考方向 $\hat{\B{s}}_0$，
通常为目标的中心，$u$ 轴向东，$v$ 轴向北.
于是，\ac{baseline}矢量为 $\B{b} = (u,v,w) \lambda$，
方向矢量为 $\hat{\B{s}} = \left( l, m, \sqrt{1-l^2-m^2} \right)$，
其中 $l, m$ 分别为 $\hat{\B{s}}$ 对 $u, v$ 轴的投影长度，即\ac{dc}.
再利用 $\D{\Omega} = \D{l}\,\D{m} \,\big/ \sqrt{1-l^2-m^2}$，
可将\autoref{eq:vis} 表示为：
\begin{equation}
  \label{eq:vis-uvw}
  \acs{Vis}(u,v,w) = \iint \frac{\ac{I-nu}(l,m)}{\sqrt{1-l^2-m^2}}
    ...
    \,\D{l}\,\D{m}.
\end{equation}
注意，因为上式的相位因子中存在额外项 \enquote{$w \sqrt{1-l^2-m^2}$}，
...
\begin{figure}[htp]
  ...
  \bicaption[$uv$ 覆盖样例]{%
    ...
    \emph{Left to right:} the integration time is \SI{10}{\second},
...
因为干涉阵列的天线数目是有限的，\ac{baseline}的长度范围也是有限的，
...
\begin{equation}
  S_{\nu} = \frac{2\,\ac{kb} \Delta T}{A_e} ,
\end{equation}
相应的测量误差为：
\begin{align}
  \sigma_S
    & = \frac{2\,\ac{kb}}{A_e} \sigma(\ac{T-antenna} + \Delta T) \\
    & \approx \frac{2\,\ac{kb}}{A_e} \sigma(\ac{T-antenna}) \\
    & = \frac{2\,\ac{kb} \ac{T-antenna}}{A_e \sqrt{\Delta\nu \,\tau}} .
\end{align}
此即单天线的\emph{点源灵敏度}.
...
\begin{equation}
  ...
  \sigma_S
    = \frac{2\,\ac{kb} T_s}{
        A_e \sqrt{\ac{N-ant}(\ac{N-ant}-1) \Delta\nu \,\tau}} .
\end{equation}

如果观测一个\ac{src-extended}，则需要考虑干涉仪的\emph{亮度灵敏度} $\sigma_b$，
