%%
%% Copyright (c) 2018-2019 Weitian LI <liweitianux@sjtu.edu.cn>
%% Creative Commons BY 4.0
%%

\DeclareAcronym{21cmline}{
  short = {21\,cm~谱线},
  long = {21\,cm line},
  extra = of neutral hydrogen,
  sort = 21-li-mi-pu-xian,
  first-style = short,
  class = glossary,
}

\DeclareAcronym{ab}{
  short = 天线波束,
  long = antenna beam,
  sort = tian-xian-bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{accretion}{
  short = 吸积,
  long = accretion,
  sort = xi-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ae}{
  short = 自编码器,
  long = autoencoder,
  sort = zi-bian-ma-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ai}{
  short = 人工智能,
  long = artificial intelligence,
  sort = ren-gong-zhi-neng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{am}{
  short = 调幅,
  long = amplitude modulation,
  extra = {\normalfont AM},
  sort = tiao-fu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{as}{
  short = 综合孔径,
  long = aperture synthesis,
  sort = zong-he-kong-jing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{backprop}{
  short = 反向传播,
  long = back-propagation,
  sort = fan-xiang-chuan-bo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{baseline}{
  short = 基线,
  long = baseline,
  sort = ji-xian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{batch-norm}{
  short = 批标准化,
  long = batch normalization,
  sort = pi-gui-fan-hua,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{batch-size}{
  short = 批大小,
  long = batch size,
  sort = pi-da-xiao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{bbn}{
  short = 太初核合成,
  long = Big Bang Nucleosynthesis,
  sort = tai-chu-he-he-cheng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{bbt}{
  short = 大爆炸理论,
  long = Big Bang Theory,
  sort = da-bao-zha-li-lun,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{beam}{
  short = 波束,
  long = beam,
  sort = bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{beam-clean}{
  short = 洁波束,
  long = clean beam,
  sort = jie-bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{beam-forming}{
  short = 波束成形,
  long = beam-forming,
  sort = bo-shu-cheng-xing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{beam-primary}{
  short = 初级波束,
  long = primary beam,
  sort = chu-ji-bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{beam-station}{
  short = 站点波束,
  long = station beam,
  sort = zhan-dian-bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{beam-synthesized}{
  short = 综合波束,
  long = synthesized beam,
  sort = zong-he-bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{bias}{
  short = 偏置,
  long = bias,
  sort = pian-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{blazar}{
  short = 耀变体,
  long = blazar,
  sort = yao-bian-ti,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{bw-smear}{
  short = 带宽涂污,
  long = bandwidth smearing,
  sort = dai-kuan-tu-wu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{c-correlator}{
  short = 复相关器,
  long = complex correlator,
  sort = fu-xiang-guan-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{cd}{
  short = 宇宙黎明,
  long = Cosmic Dawn,
  sort = yu-zhou-li-ming,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{cdf}{
  short = 累积分布函数,
  long = cumulative distribution function,
  sort = lei-ji-fen-bu-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{cell}{
  short = 格子,
  long = cell,
  sort = ge-zi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{channel}{
  short = 通道,
  long = channel,
  sort = tong-dao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{chromatic-effect}{
  short = 色彩效应,
  long = chromatic effect,
  sort = se-cai-xiao-ying,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{clustering-effect}{
  short = 成团效应,
  long = clustering effect,
  sort = cheng-tuan-xiao-ying,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{code}{
  short = 编码,
  long = code,
  sort = bian-ma,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{coi}{
  short = 影响锥,
  long = cone of influence,
  sort = ying-xiang-zhui,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{coll-deexcitation}{
  short = 碰撞退激,
  long = collisional de-excitation,
  sort = peng-zhuang-tui-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{coll-excitation}{
  short = 碰撞激发,
  long = collisional excitation,
  sort = peng-zhuang-ji-fa,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{colorbar}{
  short = 色条,
  long = color bar,
  sort = se-tiao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{column-density}{
  short = 柱密度,
  long = column density,
  sort = zhu-mi-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{comoving-frame}{
  short = 共动参考系,
  long = comoving frame,
  sort = gong-dong-can-kao-xi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{confusion}{
  short = 混淆,
  long = confusion,
  sort = hun-xiao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{conjugate}{
  short = 共轭,
  long = conjugate,
  sort = gong-e,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{contour}{
  short = 等值线,
  long = contour,
  sort = deng-zhi-xian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{conv-theorem}{
  short = 卷积定理,
  long = convolution theorem,
  sort = juan-ji-ding-li,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{correlator}{
  short = 相关器,
  long = correlator,
  sort = xiang-guan-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{cr}{
  short = 宇宙射线,
  long = cosmic ray,
  sort = yu-zhou-she-xian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{cwt}{
  short = 连续小波变换,
  long = continuous wavelet transform,
  sort = lian-xu-xiao-bo-bian-huan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{da}{
  short = 黑暗时期,
  long = Dark Ages,
  sort = hei-an-shi-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dae}{
  short = 去噪自编码器,
  long = denoising autoencoder,
  sort = qu-zao-zi-bian-ma-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dbn}{
  short = 深度置信网络,
  long = deep belief network,
  sort = shen-du-zhi-xin-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dc}{
  short = 方向余弦,
  long = direction cosine,
  sort = fang-xiang-yu-xian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{decoder}{
  short = 解码器,
  long = decoder,
  sort = jie-ma-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{deconv}{
  short = 解卷积,
  long = deconvolution,
  sort = jie-juan-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{delay-center}{
  short = 延迟中心,
  long = delay center,
  sort = yan-chi-zhong-xin,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{delay-spec}{
  short = 延迟谱,
  long = delay spectrum,
  sort = yan-chi-pu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{denoising}{
  short = 去噪,
  long = denoising,
  sort = qu-zao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{detailed-balance}{
  short = 细致平衡,
  long = detailed balance,
  sort = xi-zhi-ping-heng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dim-reduction}{
  short = 降维,
  long = dimensionality reduction,
  sort = jiang-wei,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{directivity}{
  short = 指向性,
  long = directivity,
  sort = zhi-xiang-xing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dl}{
  short = 深度学习,
  long = deep learning,
  sort = shen-du-xue-xi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dm}{
  short = 暗物质,
  long = dark matter,
  sort = an-wu-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dnn}{
  short = 深度神经网络,
  long = deep neural network,
  sort = shen-du-shen-jing-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dod}{
  short = 简并度,
  long = degree of degeneracy,
  sort = jian-bing-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ds}{
  short = 数据集,
  long = dataset,
  sort = shu-ju-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dual}{
  short = 对偶,
  long = dual,
  extra = in Fourier Transform,
  sort = dui-ou,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{dynamic-range}{
  short = 动态范围,
  long = dynamic range,
  sort = dong-tai-fan-wei,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{e-primary}{
  short = 初级电子,
  long = primary electron,
  sort = chu-ji-dian-zi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{em-spontaneous}{
  short = 自发发射,
  long = spontaneous emission,
  sort = zi-fa-fa-she,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{em-stimulated}{
  short = 受激发射,
  long = stimulated emission,
  sort = shou-ji-fa-she,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{encoder}{
  short = 编码器,
  long = encoder,
  sort = bian-ma-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{envelope}{
  short = 包络,
  long = envelope,
  sort = bao-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{epoch}{
  short = 周期,
  long = epoch,
  sort = zhou-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{equipartition}{
  short = 均分,
  long = equipartition,
  sort = jun-fen,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{exosphere}{
  short = 散逸层,
  long = exosphere,
  sort = san-yi-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-activation}{
  short = 激活函数,
  long = activation function,
  sort = ji-huo-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-error}{
  short = 误差函数,
  long = error function,
  sort = wu-cha-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-flux}{
  short = 流量函数,
  long = flux function,
  sort = liu-liang-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-luminosity}{
  short = 光度函数,
  long = luminosity function,
  sort = guang-du-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-sampling}{
  short = 采样函数,
  long = sampling function,
  sort = cai-yang-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-tanh}{
  short = 双曲正切函数,
  long = hyperbolic tangent function,
  sort = shuang-qu-zheng-qie-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-weighting}{
  short = 权重函数,
  long = weighting function,
  sort = quan-zhong-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{f-window}{
  short = 窗函数,
  long = window function,
  sort = chuang-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fcn}{
  short = 全卷积网络,
  long = fully convolutional network,
  sort = quan-juan-ji-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fg-avd}{
  short = 前景回避,
  long = foreground avoidance,
  sort = qian-jing-hui-bi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fg-rm}{
  short = 前景扣除,
  long = foreground removal,
  sort = qian-jing-kou-chu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fg-wedge}{
  short = 前景楔形,
  long = foreground wedge,
  sort = qian-jing-xie-xing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{filter}{
  short = 过滤器,
  long = filter,
  extra = in convolutional layer,
  sort = gou-lv-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{finite-diff-scheme}{
  short = 有限差分法,
  long = finite difference scheme,
  sort = you-xian-cha-fen-fa,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fm}{
  short = 调频,
  long = frequency modulation,
  extra = {\normalfont FM},
  sort = tiao-pin,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fringe}{
  short = 条纹,
  long = fringe,
  sort = tiao-wen,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fv}{
  short = 特征矢量,
  long = feature vector,
  sort = te-zheng-shi-liang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{fx}{
  short = 特征提取,
  long = feature extraction,
  sort = te-zheng-ti-qu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{g-units}{
  short = 高斯单位制,
  long = Gaussian units,
  sort = gao-si-dan-wei-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{galactic-wind}{
  short = 星系风,
  long = galactic wind,
  sort = xing-xi-feng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{gain}{
  short = 增益,
  long = gain,
  sort = zeng-yi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{gan}{
  short = 生成对抗网络,
  long = generative adversarial network,
  sort = sheng-cheng-dui-kang-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{gc}{
  short = 星系团,
  long = galaxy cluster,
  sort = xing-xi-tuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{gl-strong}{
  short = 强引力透镜,
  long = strong gravitational lensing,
  sort = qiang-yin-li-tou-jing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{grid}{
  short = 栅格,
  long = grid,
  sort = shan-ge,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{gridding}{
  short = 栅格化,
  long = gridding,
  sort = shan-ge-hua,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ground-state}{
  short = 基态,
  long = ground state,
  sort = ji-tai,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{gw}{
  short = 引力波,
  long = gravitational wave,
  sort = yin-li-bo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hi}{
  short = 中性氢,
  long = neutral hydrogen,
  extra = {\normalfont H\textsc{i}},
  sort = zhong-xing-qing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hier-clustering}{
  short = 等级式成团,
  long = hierarchical clustering,
  sort = deng-ji-shi-cheng-tuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hii}{
  short = 电离氢,
  long = ionized hydrogen,
  extra = {\normalfont H\textsc{ii}},
  sort = dian-li-qing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hotspot}{
  short = 热斑,
  long = hot spot,
  sort = re-ban,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hourangle}{
  short = 时角,
  long = hour angle,
  sort = shi-jiao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hyperfine-splitting}{
  short = 超精细分裂,
  long = hyperfine splitting,
  sort = chao-jing-xi-fen-lie,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{hyperparam}{
  short = 超参数,
  long = hyperparameter,
  sort = chao-can-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{igm}{
  short = 星系际介质,
  long = intergalactic medium,
  sort = xing-xi-ji-jie-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{interferometry}{
  short = 干涉测量,
  long = interferometry,
  sort = gan-she-ce-liang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ism}{
  short = 星际介质,
  long = intersteller medium,
  sort = xi-ji-jie-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{imgcube}{
  short = 图像立方,
  long = image cube,
  sort = tu-xiang-li-fang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{inflation}{
  short = 暴胀,
  long = inflation,
  sort = bao-zhang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ionosphere}{
  short = 电离层,
  long = ionosphere,
  sort = dian-li-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{jet}{
  short = 喷流,
  long = jet,
  sort = pen-liu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{laser}{
  short = 激光,
  long = laser,
  extra = light amplification by stimulated emission of radiation,
  sort = ji-guang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{layer-conv}{
  short = 卷积层,
  long = convolutional layer,
  sort = juan-ji-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{layer-fc}{
  short = 全连接层,
  long = fully connected layer,
  sort = quan-lian-jie-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{layer-hidden}{
  short = 隐藏层,
  long = hidden layer,
  sort = ying-cang-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{layer-pooling}{
  short = 池化层,
  long = pooling layer,
  sort = chi-hua-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{layer-upsampling}{
  short = 上采样层,
  long = upsampling layer,
  sort = shang-cai-yang-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{learning-rate}{
  short = 学习速率,
  long = learning rate,
  sort = xue-xi-shu-lv,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{lightcone}{
  short = 光锥,
  long = light cone,
  sort = guang-zhui,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{line-profile}{
  short = 谱线轮廓,
  long = line profile,
  sort = pu-xian-lun-kuo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{loop-gain}{
  short = 循环增益,
  long = loop gain,
  extra = in CLEAN algorithm,
  sort = xun-huan-zeng-yi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{lsf}{
  short = 大尺度纤维状结构,
  long = large-scale filaments,
  sort = da-chi-du-xian-wei-zhuang-jie-gou,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{lte}{
  short = 局部热力学平衡,
  long = local thermodynamic equilibrium,
  sort = ju-bu-re-li-xue-ping-heng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{m-major}{
  short = 主并合,
  long = major merger,
  sort = zhu-bing-he,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{m-minor}{
  short = 次并合,
  long = minor merger,
  sort = ci-bing-he,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{m-tree}{
  short = 并合树,
  long = merger tree,
  sort = bing-he-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{magnetosphere}{
  short = 磁层,
  long = magnetosphere,
  sort = ci-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mag-moment}{
  short = 磁矩,
  long = magnetic moment,
  sort = ci-gu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mainlobe}{
  short = 主瓣,
  long = main lobe,
  sort = zhu-ban,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{map-clean}{
  short = 洁图,
  long = clean map,
  sort = jie-tu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{map-dirty}{
  short = 脏图,
  long = dirty map,
  sort = zang-tu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{map-model}{
  short = 模型图,
  long = model map,
  sort = mo-xing-tu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{map-residual}{
  short = 残差图,
  long = residual map,
  sort = can-cha-tu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{maser}{
  short = 脉泽,
  long = maser,
  extra = microwave amplification by stimulated emission of radiation,
  sort = mai-ze,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mcf}{
  short = 互相干函数,
  long = mutual coherence function,
  sort = hu-xiang-gan-han-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mem}{
  short = 最大熵方法,
  long = maximum entropy method,
  sort = zui-da-shang-fang-fa,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mesosphere}{
  short = 中间层,
  long = mesosphere,
  sort = zhong-jian-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mfluid}{
  short = 磁流体,
  long = magneto-fluid,
  sort = ci-liu-ti,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mfp}{
  short = 平均自由程,
  long = mean free path,
  sort = ping-jun-zi-you-cheng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ml}{
  short = 机器学习,
  long = machine learning,
  sort = ji-qi-xue-xi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mode}{
  short = 模式,
  long = mode,
  sort = mo-shi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mode-mixing}{
  short = 模式混合,
  long = mode mixing,
  sort = mo-shi-hun-he,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{mse}{
  short = 均方误差,
  long = mean squared error,
  sort = jun-fang-wu-cha,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{neuron}{
  short = 神经元,
  long = neuron,
  sort = shen-jing-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{nn}{
  short = 神经网络,
  long = neural network,
  sort = shen-jing-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{no-flux}{
  short = 无流量,
  long = no-flux,
  extra = boundary condition,
  sort = wu-liu-liang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{occlusion}{
  short = 遮蔽,
  long = occlusion,
  sort = zhe-bi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{overdensity}{
  short = 过密度,
  long = overdensity,
  sort = gou-mi-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{overfitting}{
  short = 过拟合,
  long = overfitting,
  sort = gou-ni-he,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{outlier}{
  short = 离群值,
  long = outlier,
  sort = li-qun-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{passband}{
  short = 通带,
  long = passband,
  sort = tong-dai,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{percentile}{
  short = 百分位点,
  long = percentile,
  sort = bai-fen-wei-dian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{phase-refpos}{
  short = 相位参考位置,
  long = phase reference position,
  sort = xiang-wei-can-kao-wei-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{phased-array}{
  short = 相控阵,
  long = phased array,
  sort = xiang-kong-zhen,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{pitch-angle}{
  short = 螺距角,
  long = pitch angle,
  sort = luo-ju-jiao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{pl}{
  short = 偏振泄漏,
  long = polarization leakage,
  sort = pian-zhen-xie-lou,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{pp}{
  short = 功率方向图,
  long = power pattern,
  sort = gong-lv-fang-xiang-tu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{pr-cond}{
  short = 条件概率,
  long = conditional probability,
  sort = tiao-jian-gai-lv,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{progenitor}{
  short = 前身,
  long = progenitor,
  sort = qian-shen,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{propagator}{
  short = 传播子,
  long = propagator,
  sort = chuan-bo-zi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ps}{
  short = 功率谱,
  long = power spectrum,
  sort = gong-lv-pu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{pulsar}{
  short = 脉冲星,
  long = pulsar,
  sort = mai-chong-xing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{quasar}{
  short = 类星体,
  long = quasar,
  extra = quasi-stellar radio source,
  sort = lei-xing-ti,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rad-brem}{
  short = 轫致辐射,
  long = bremsstrahlung,
  sort = ren-zhi-fu-she,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rad-ff}{
  short = 自由—自由辐射,
  long = free--free radiation,
  sort = zi-you-zi-you-fu-she,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rad-syn}{
  short = 同步辐射,
  long = synchrotron radiation,
  sort = tong-bu-fu-she,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{radiometer}{
  short = 辐射计,
  long = radiometer,
  sort = fu-she-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ram-pressure}{
  short = 冲压,
  long = ram pressure,
  sort = chong-ya,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ram-pressure-strip}{
  short = 冲压剥离,
  long = ram-pressure stripping,
  sort = chong-ya-bao-li,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{reciprocity-theorem}{
  short = 互易定理,
  long = reciprocity theorem,
  sort = hu-yi-ding-li,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{recombination}{
  short = 复合,
  long = recombination,
  sort = fu-he,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{regularizer}{
  short = 正则化项,
  long = regularizer,
  sort = zheng-ze-hua-xiang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{reionization}{
  short = 再电离,
  long = reionization,
  sort = zai-dian-li,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{relu}{
  short = 修正线性单元,
  long = rectified linear unit,
  sort = xiu-zheng-xian-xing-dan-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{representation}{
  short = 表示,
  long = representation,
  sort = biao-shi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{res-nn}{
  short = 残差神经网络,
  long = residual neural network,
  sort = can-cha-shen-jing-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{resonant-scattering}{
  short = 共振散射,
  long = resonant scattering,
  sort = gong-zhen-san-she,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rh}{
  short = 射电晕,
  long = radio halo,
  sort = she-dian-yun,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rmh}{
  short = 微射电晕,
  long = radio mini-halo,
  sort = mi-ni-she-dian-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rms}{
  short = 方均根,
  long = root mean square,
  sort = fang-jun-gen,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rnn}{
  short = 循环神经网络,
  long = recurrent neural network,
  sort = xun-huan-shen-jing-wang-luo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rr}{
  short = 射电遗迹,
  long = radio relic,
  sort = she-dian-yi-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rrl}{
  short = 射电复合线,
  long = radio recombination line,
  sort = she-dian-fu-he-xian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{rt}{
  short = 辐射转移,
  long = radiative transfer,
  sort = fu-she-zhuan-yi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{sampling-theorem}{
  short = 采样定理,
  long = sampling theorem,
  sort = cai-yang-di-li,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{sc}{
  short = 超星系团,
  long = supercluster,
  sort = chao-xing-xi-tuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{scaling-relation}{
  short = 标度关系,
  long = scaling relation,
  sort = biao-du-guan-xi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{sft}{
  short = 自旋翻转跃迁,
  long = spin-flip transition,
  sort = zi-xuan-fan-zhuan-yue-qian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{sidelobe}{
  short = 旁瓣,
  long = side lobe,
  sort = pang-ban,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{singlet}{
  short = 单态,
  long = singlet,
  sort = dan-yi-tai,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{skymap}{
  short = 天图,
  long = sky map,
  sort = tian-tu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{slice}{
  short = 切片,
  long = slice,
  sort = qie-pian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{sloshing}{
  short = 晃荡,
  long = sloshing,
  sort = huang-dang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{snapshot}{
  short = 快照,
  long = snapshot,
  sort = kuai-zhao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{sn-remnant}{
  short = 超新星遗迹,
  long = supernova remnant,
  sort = chao-xin-xing-yi-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{snr}{
  short = 信噪比,
  long = signal-to-noise ratio,
  sort = xin-zao-bi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spec-brightness}{
  short = 谱亮度,
  long = spectral brightness,
  sort = pu-liang-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spec-energy}{
  short = 谱能量,
  long = spectral energy,
  sort = pu-neng-liang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spec-line}{
  short = 谱线,
  long = spectral line,
  sort = pu-xian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spec-power}{
  short = 谱功率,
  long = spectral power,
  sort = pu-gong-lv,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spherical-collapse}{
  short = 球形坍缩,
  long = spherical collapse,
  sort = qiu-xing-ta-suo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spillover}{
  short = 漏失,
  long = spillover,
  sort = lou-shi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{spin}{
  short = 自旋,
  long = spin,
  sort = zi-xuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{src-compact}{
  short = 致密源,
  long = compact source,
  sort = zhi-mi-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{src-discrete}{
  short = 离散源,
  long = discrete source,
  sort = li-san-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{src-extended}{
  short = 展源,
  long = extended source,
  sort = zhan-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{src-peeling}{
  short = 剥源,
  long = source peeling,
  sort = bao-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{src-point}{
  short = 点源,
  long = point source,
  sort = dian-yuan,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{station}{
  short = 站点,
  long = station,
  sort = zhan-dian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{T-color}{
  short = 色温度,
  long = color temperature,
  sort = se-wen-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{t-avg}{
  short = 平均时间,
  long = averaging time,
  sort = ping-jun-shi-jian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{t-int}{
  short = 积分时间,
  long = integration time,
  sort = ji-fen-shi-jian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{t-smear}{
  short = 时间涂污,
  long = time smearing,
  sort = shi-jian-tu-wu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{t-transit}{
  short = 渡越时间,
  long = transit time,
  sort = du-yue-shi-jian,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{te}{
  short = 热力学平衡,
  long = thermodynamic equilibrium,
  sort = re-li-xue-ping-heng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{thermosphere}{
  short = 热层,
  long = thermosphere,
  sort = re-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{timescale}{
  short = 时标,
  long = timescale,
  sort = shi-biao,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{tomography}{
  short = {层析成像},
  long = {tomography},
  sort = ceng-xi-cheng-xiang,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{transmittance}{
  short = 透射率,
  long = transmittance,
  sort = tou-she-lv,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{transpose}{
  short = 转置,
  long = transpose,
  sort = zhuan-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{triplet}{
  short = 三重态,
  long = triplet,
  sort = san-chong-tai,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{troposphere}{
  short = 对流层,
  long = troposphere,
  sort = dui-liu-ceng,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{ttd}{
  short = 渡越时间阻尼,
  long = transit-time damping,
  sort = du-yue-shi-jian-zu-ni,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{turbulence}{
  short = 湍流,
  long = turbulence,
  sort = tuan-liu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{turbreacc-model}{
  short = 湍流再加速模型,
  long = turbulent re-acceleration model,
  sort = tuan-liu-zai-jia-su-mo-xing,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{unsupervised}{
  short = 无监督,
  long = unsupervised,
  extra = learning,
  sort = wu-jian-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{V-comoving}{
  short = 共动体积,
  long = comoving volume,
  sort = gong-dong-ti-ji,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{v-peculiar}{
  short = 本动速度,
  long = peculiar velocity,
  sort = ben-dong-su-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{v-proper}{
  short = 自行速度,
  long = proper velocity,
  sort = zi-xing-su-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{vae}{
  short = 变分自编码器,
  long = variational autoencoder,
  sort = bian-fen-zi-bian-ma-qi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{vis}{
  short = 可见度,
  long = visibility,
  sort = ke-jian-du,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{w-proj}{
  short = {$w$ 投影},
  long = $w$-projection,
  sort = w-tou-ying,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{w-stack}{
  short = {$w$ 堆叠},
  long = $w$-stacking,
  sort = w-dui-die,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{wave-long}{
  short = 纵波,
  long = longitudinal wave,
  sort = zong-bo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{wave-ms}{
  short = 磁声波,
  long = magnetosonic wave,
  sort = ci-sheng-bo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{wave-trans}{
  short = 横波,
  long = transverse wave,
  sort = heng-bo,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{wavenumber}{
  short = 波数,
  long = wave number,
  extra = in Fourier domain,
  sort = bo-shu,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{wim}{
  short = 温电离介质,
  long = warm ionized medium,
  sort = wen-dian-li-jie-zhi,
  first-style = reversed,
  class = glossary,
}

\DeclareAcronym{zenith}{
  short = 天顶,
  long = zenith,
  sort = tian-ding,
  first-style = reversed,
  class = glossary,
}


\endinput
