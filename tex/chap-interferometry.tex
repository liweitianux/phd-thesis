...
\begin{itemize}
  ...
  \item 源自宇宙再电离时期 (红移 $z \sim \numrange{6}{16}$)
    的\acl{hi} \hisignal/历经红移后将出现在低频波段，因此不可避免地需要在
...
研发更有效的数据处理方法（参见 \autoref{sec:det-methods}）.
...
\end{equation}
可见，一个天线的\acl{gain}与其\acl{pp}只相差一个常数.

%---------------------------------------------------------------------
\subsection{主瓣}

天线的\acl{pp} $P(\theta,\phi)$ 通常会在一定方向范围明显大于在其他方向的值，
